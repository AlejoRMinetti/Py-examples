...
def involved(courses, person):
    res = {}
    for hexamester, HexaCourses in courses.items() :
        cursosEnHexa = []
        for course in HexaCourses:
            if person in HexaCourses[course].get("teacher") == person or HexaCourses[course].get("assistant") == person:
                cursosEnHexa.append(course)
        if cursosEnHexa:
            res[hexamester] = cursosEnHexa
